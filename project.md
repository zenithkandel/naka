...
* **GPU:** NVIDIA RTX 3060 (12 GB VRAM). *Note: Minimum spec requires all models to be exported to TensorRT FP16 to achieve real-time 30 FPS processing.*
...
* **Databases:** PostgreSQL 15+ (Relational), Qdrant (Vector).
...
* **Detection:** YOLOv8x runs on every frame to detect persons and carried items. The system natively supports three COCO classes: `backpack`, `handbag`, and `suitcase`. *(Note: 'Duffel bag' requires a custom fine-tuned model as it is not a standard COCO class).*
...
* **Encryption:** AES-256 encryption at rest is enforced for the PostgreSQL database, and TLS 1.3 is required for all API and web traffic in transit.
